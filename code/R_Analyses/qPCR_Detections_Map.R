...
qpcr.sum <- read.csv("data/2024Perley/qPCR/APC0264 qPCR Results 06-Nov-24-Summary.csv", header = T) %>% dplyr::select(SampleID, ABL_Sample_ID, Station, Date, Replicate, eDNA_Concentration, A..lupus, G..morhua, H..hippoglossus) %>% filter(Replicate==1) %>% as.data.frame()
...
qpcr.long <- pivot_longer(data = qpcr.results, cols = A..lupus:H..hippoglossus, names_to = "Species")
